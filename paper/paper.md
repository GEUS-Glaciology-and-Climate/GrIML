...
![An overview of the Greenland ice-marginal lake inventory series, which maps the number of lakes and their size between the years of 2016 and 2023 [@how_data_2025]. The map shows the location of each lake, with the symbol signifying area change over time, where green circles represent an increase in lake area and yellow squares represent a decrease in lake area. The bar graphs show change in the number of lakes over each region  \label{fig:inventory}](https://github.com/GEUS-Glaciology-and-Climate/GrIML/blob/features/new-docs/docs/figures/iml_dataset_overview.png?raw=true)
